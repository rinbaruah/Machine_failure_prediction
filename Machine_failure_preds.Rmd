...
```{r}
fitted.results <- predict(model_logit,newdata=subset(train,select=-(5)),type='response')
...
```{r}
p <- predict(model_logit, newdata=train, type="response")
pr <- prediction(p, train$machine_failure)
